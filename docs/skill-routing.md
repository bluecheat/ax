...
| 진단·관리 | `doctor`, `rules`, `spirit`, `hud` |
| 도입 | `up`, `onboarding` |
| Mistake Loop | `mistake` (캡처), `audit` (회고·승격) |
| 진입점 | `triage` |
| 결정 기록 | `adr` |

`skills/<name>/SKILL.md` 는 `.claude-plugin` 플러그인이 자동 로드해요 — 프로젝트 파일이 아니라 plugin 자산. `install` skill 은 `up` 으로 개명됐고, `spec-plan` 은 폐기됐어요 (설계 결정은 ADR 로 이동).
...
그 외 모든 기능은 자연어로 부르면 돼요: "goax 도입해줘" / "spec 만들어줘 — payment-refund" / "tasks 분해" / "구현 시작" / "spec 확인" / "ADR 작성" / "audit 실행" / "실수 기록해줘" / "rules 보여줘" / "진단해줘" / "spirit 점검" 등. 자동 라우팅이 안 잡히면 `/goax` 인덱스의 키워드를 참고하거나 `Skill goax:<name>` 으로 명시 호출해요.
