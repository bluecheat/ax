...
goax 의 모든 기능은 **자연어 트리거** 로 호출돼요 (0.1.20+: 13개 thin wrapper 폐기, /goax 인덱스 1개만 유지). 사용자가 `/goax` 만 쳤을 때 다음 인덱스를 보여주세요. 또한 `.ax/state.json` 이 있으면 마지막에 현재 4계층 활성도를 한 줄로 보여주세요.
