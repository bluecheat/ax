...
#
# Usage:
#   bash mark-task.sh [--spec <NNN-slug>] --task T013 [--state x|~] [--dry-run] [--json]
#   bash mark-task.sh [--spec <NNN-slug>] --next [--json]
#
# 왜 필요한가 — spec-implement §5 가 인라인 sed 로 체크박스를 켰어요:
#   sed -E "/^- \[ \] .*T013([^0-9A-Za-z]|$)/ s/^- \[ \]/- [x]/"
# `.*` 때문에 "줄 어디에든" ID 가 있으면 매칭돼서, 본문에 다른 task 를 언급한 줄
# ("T009 가 셋 다 해소" · "T005 의 상수와 맞춤") 까지 같이 켜졌어요 — 실측 3건이 검증 없이
# 완료로 바뀌었고 게이트는 그걸 정상으로 읽었어요. 그리고 그 sed 는 락이 없어서
# 같은 파일을 쓰는 lanes-dispatch.sh·tasks-gate.sh 와 lost-update 가 날 수 있었어요.
#
# 이 스크립트는 셋을 고쳐요:
#   1. 매칭은 **줄 앞** — `- [ ] T013 ` / `- [ ] **T013**` / `- [ ] [T013]` 만. 본문 언급은 안 봐요
#   2. 코드 펜스(```) 안 줄은 건너뛰어요 — 템플릿 예시 `- [ ] T001 [P] [AC2] <한 줄 설명>` 이 걸리지 않게
#   3. tasks.md.lock 을 잡고 쓰고, 쓰기 전후 `[x]` 개수 차이가 정확히 1(또는 idempotent 0)인지 확인해요
#
# --next 는 읽기 전용 — 펜스 밖 첫 미완료 task 줄을 돌려줘요 (spec-implement §1 의 NEXT_TASK).
#
# Output (--json):
...
PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$(find_project_root 2>/dev/null || pwd)}"
