# .claude/hooks — Sensors

PDF의 Sensor Taxonomy를 자동화한다.
...
`.ax-first/config.yml`의 `sensors.mode`:
...
```bash
ln -sf "$(pwd)/.claude/hooks/pre-commit/critical-rule-grep.sh" .git/hooks/pre-commit
```
