...
3. **승격**: N회 이상 반복된 카테고리 → 집행 사다리 위에서부터 — ① 구조로 제거 → ② 기존 lint/CI(`external:`) → ③ grep 패턴 → ④ 산문 룰. ③ 은 룰 아래 `<!-- 검출 패턴: <ERE> -->` 한 줄이고, `critical-rule-grep.sh` 가 그 줄을 읽어 pre-commit 에서 실제로 막아요 — 승격과 집행이 같은 커밋이에요. 산문 룰은 모델이 잊을 수 있는 soft 층이라 마지막이에요
...
- [`templates/default/.ax/scripts/bash/README.md`](templates/default/.ax/scripts/bash/README.md) — 결정론 도구 42개 표준
