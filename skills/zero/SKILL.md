...
ID 는 손으로 고르지 않아요 — `--reserve` 가 날짜+난수(`YYYY-MM-DD-<4hex>`) ID 를 뽑아 파일까지 만들어요.
순번이 아니라서 다른 세션·브랜치의 ADR 과 겹치지 않아요. 반환된 `.result.path` 를 그대로 쓰세요.

**산출물**: `.ax/docs/adr/<id>-*.md` … (보통 3~6개) + 미결정 축 목록
