...
# 맨 grep 을 쓰지 않아요 — 템플릿 "한 줄 형식" 코드 펜스 안의 예시(`- [ ] T001 [P] [AC2] <한 줄 설명>`)를
# 다음 task 로 집어요. mark-task.sh --next 는 펜스 밖 줄만 봐요.
NEXT=$(bash .ax/scripts/bash/mark-task.sh --spec "$SPEC" --next --json)
NEXT_TASK=$(printf '%s' "$NEXT" | jq -r '.result.task // empty')
...
# 체크박스는 mark-task.sh 가 켜요 — 줄 **앞** 의 ID 만 보고(`- [ ] T013 `·`**T013**`·`[T013]`),
# 코드 펜스 안은 건너뛰고, tasks.md.lock 을 잡고, 쓰기 전후 [x] 개수가 정확히 1 늘었는지 확인해요.
# 인라인 sed(`.*T013`)로 하면 본문에 다른 task 를 언급한 줄("T009 가 해소")까지 켜져요 — 실측 3건.
bash .ax/scripts/bash/mark-task.sh --spec "$SPEC" --task "$TASK_ID" --json
# 의도적 보류는 --state '~' (사유는 task 줄에 `보류: <사유>` 로 손으로 적어요)
```
