...
- [0.7.2](0.7.2.md) — task 하나를 끝냈는데 본문에 이름만 나온 다른 task 까지 완료로 바뀌던 것 (`mark-task.sh`)
- [0.7.1](0.7.1.md) — lint 결과가 모델에게 닿고, 품질 설정 완화는 이유부터, 압축 뒤에도 위치를 기억해요 · 응답 이모지 정리
- [0.7.0](0.7.0.md) — 룰을 읽기 전엔 편집을 막고, 되돌리기 어려운 명령은 사실부터 적게 해요 · spec/ADR ID 가 브랜치끼리 안 겹쳐요
...
- [0.5.7](0.5.7.md) — 안전망이 반쪽만 켜져 있던 자리 넷: 시크릿 SSOT · 파일 쓰기 락 6곳 · zero-verify 주석 파서 · warning 회귀 (+ 리눅스 stat·멀티바이트 브래킷)
- [0.5.6](0.5.6.md) — 리눅스에서 placeholder 게이트가 조용히 꺼져 있던 것 (GNU grep 의 `[가-힣]` 범위 거부)
- [0.5.5](0.5.5.md) — 게이트가 서 있는 바닥: 원장 락 · 템플릿이 게이트를 통과하게 · 막히면 닫히게 (멀티모델 리뷰 P0 11·P1 19 반영)
