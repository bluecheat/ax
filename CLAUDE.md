...
- Skills update `.ax/state.json` (HUD signals) and `.ax/current-task.json` (triage→spec→audit context handoff) at well-defined points. Use the existing helpers — `update-state.sh` for `state.json`, **`update-task.sh` for `current-task.json` task fields** (`--phase` · `--set k=v` · `--blocked-by` · `--merge-intent` · `--start`), `status-note.sh` for its `handoff` — and do not invent new state files. SKILL.md must not write `current-task.json` with inline jq (`tests/smoke.sh` fails the build): the three script writers share one lock string, an inline writer has none, and a whole-object rebuild silently drops `handoff`.
