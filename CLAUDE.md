...
- When a script does `read → jq/awk → tmp.$$ → mv` on a file more than one script or session can touch concurrently (`tasks.md`, `state.json`, `STATUS.md`), wrap the read-modify-write in `common.sh`'s `goax_lock "$FILE.lock"` / `goax_unlock` — without it, concurrent writers silently lose each other's updates. Two scripts writing the *same* file (e.g. `update-state.sh` and `tasks-gate.sh` both touch `state.json`) must use the identical lock path string, or the lock is split in two and does nothing.
